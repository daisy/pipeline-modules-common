...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/">
		<xsl:for-each select="distinct-values(//d:file/resolve-uri(substring-before(@href,'!/'),base-uri(.)))">
			...
			<xsl:result-document href="manifest.xml" format="zip-manifest">
				<xsl:element name="c:zip-manifest">
					...
					<xsl:apply-templates select="$fileset.zip//d:file[resolve-uri(substring-before(@href,'!/'),base-uri(.))=$href]"/>
				</xsl:element>
			</xsl:result-document>
		</xsl:for-each>
	</xsl:template>
